...
type
  TDMEstoque = class(TDataModule)
    qryRelEstoqueValidade: TFDQuery;
    qryRelEstoqueValidadeCODIGO: TIntegerField;
    qryRelEstoqueValidadeCODBARRA: TStringField;
    qryRelEstoqueValidadeDESCRICAO: TStringField;
    qryRelEstoqueValidadeREFERENCIA: TStringField;
    qryRelEstoqueValidadeUNIDADE: TStringField;
    qryRelEstoqueValidadeQTD_ATUAL: TFMTBCDField;
    qryRelEstoqueValidadePR_CUSTO: TFMTBCDField;
    qryRelEstoqueValidadePR_VENDA: TFMTBCDField;
    qryRelEstoqueValidadeTOTAL_COMPRA: TFMTBCDField;
    qryRelEstoqueValidadeTOTAL_VENDA: TFMTBCDField;
    qryRelEstoqueValidadeGRUPO_SL: TStringField;
    frxDBEstoqueValidade: TfrxDBDataset;
    frxDBEmpresa: TfrxDBDataset;
    qryRelEstoqueValidadeDATA_VALIDADE: TDateField;
  private
      ...
      begin
        ...
        begin
          ...
          begin // Estoque Real
            ...
            qryExecute.SQL.Add(' QTD_ATUAL=QTD_ATUAL - :QTD_ATUAL');
            qryExecute.SQL.Add(' where');
            qryExecute.SQL.Add(' codigo=:id and (((servico is null) or (servico=''N'')) and (composicao=''N''))');
            qryExecute.Params[0].Value := QtdEstoque;
            qryExecute.Params[1].Value := aProduto;
            qryExecute.ExecSQL;
            dados.Conexao.CommitRetaining;
...
begin
  ...
  begin
    ...
    dados.qryExecute.SQL.Text :=  'update produto_grade set qtd = qtd + :qtd1 where codigo=:cod';
...
begin
  try
    ...
    qryEstoque.SQL.Text := ' select * from COMPRA_ITENS where FK_COMPRA=:ID and QTD>0';
    ...
    begin
      ...
      begin
        if dados.qryEmpresaEXIBE_ESTOQUE_FISCAL.Value = 'S' then
          DMEstoque.AtualizaEstoque(qryEstoque.FieldByName('FK_PRODUTO').Value, qryEstoque.FieldByName('QTD').Value, 0, 'E', 'F');

        DMEstoque.AtualizaEstoque(qryEstoque.FieldByName('FK_PRODUTO').Value, qryEstoque.FieldByName('QTD').Value, 0, 'E', 'R');
      end
      else
      begin
        DMEstoque.AtualizaEstoque(qryEstoque.FieldByName('FK_PRODUTO').Value, qryEstoque.FieldByName('QTD').Value, 0, 'E', 'R');
