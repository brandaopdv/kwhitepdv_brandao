...
begin
  ...
  Qry.SQL.Add(
    ...
    '     inner join vendas_master vm on vfpg.vendas_master=vm.codigo '+
    '     where (vm.lote=c.lote) and (vm.situacao=''F'') and (vfpg.situacao=''F'') '+
    '     and (vfpg.tipo=''C'') and (vm.fk_caixa=c.codigo) and (vm.fk_usuario=u.codigo)), '+ //C-Cartão crédito

    '    (select coalesce(sum(vfpg.valor), 0) debito from vendas_fpg vfpg '+
    '     inner join vendas_master vm on vfpg.vendas_master=vm.codigo '+
    '     where (vm.lote=c.lote) and (vm.situacao=''F'') and (vfpg.situacao=''F'') '+
    '     and (vfpg.tipo=''E'') and (vm.fk_caixa=c.codigo) and (vm.fk_usuario=u.codigo)), '+ //E-Cartão débito
...
begin
  ...
  begin
    ...
    // 11-06-2024 - Não existia cálculo do ticket médio
