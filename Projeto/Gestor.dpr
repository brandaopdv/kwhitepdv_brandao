...
uses
  ...
  Themes,
  Styles,
  Vcl.Consts in '..\View\Vcl.Consts.pas',
  ...
  uConsultaMovimentacao in '..\View\uConsultaMovimentacao.pas' {FrmConsultaMovimentacao},
  uZerarBancoDados in '..\View\uZerarBancoDados.pas' {frmZerarBancoDados},
  ...
  UConfigDatabase in '..\Model\UConfigDatabase.pas' {FrmConfigDatabase},
  uConfInicial in '..\View\uConfInicial.pas' {FrmConfInicial},
  uExportImportDados in '..\View\uExportImportDados.pas' {frmExportImportDados},
  uMsgNFCePendente in '..\View\uMsgNFCePendente.pas' {frmMsgNFCePendente},
  uMsgCertificadoValidade in '..\View\uMsgCertificadoValidade.pas' {frmMsgCertificadoValidade},
  Pipedream in '..\Classes\Pipedream.pas',
  uAtualizadorIBPT in '..\View\uAtualizadorIBPT.pas' {FrmAtualizaIBPT},
  uFornecedores in '..\View\uFornecedores.pas' {frmFornecedores},
  uAtualizadorForn in '..\View\uAtualizadorForn.pas' {FrmAtualizaForn},
  PAFNFCe in '..\Classes\PAFNFCe.pas',
  uMenuFiscal in '..\View\uMenuFiscal.pas' {frmMenuFiscal},
  LicencaAPI in '..\Classes\LicencaAPI.pas';
...
begin
  // 11-01-2025 - Se for reinicio de atualização não bloqueia
  //bParam := (ParamStr(1) = '@restart');

  if {(not bParam) and} (OpenMutex(MUTEX_ALL_ACCESS, false, 'KAW WHITE GESTOR') = 0) then
    MutexHandle := CreateMutex(nil, false, 'KAW WHITE GESTOR')
  else begin
    MessageDlg('Sistema White PDV já está ativo!', mtInformation, [mbok],0);
    SetForegroundWindow(FindWindow('TApplication', 'KAW WHITE GESTOR'));
    exit;
  end;

  {CreateMutex(nil,False,'KAW WHITE GESTOR');
  if GetLastError = ERROR_ALREADY_EXISTS Then
  Begin
    SendMessage(HWND_BROADCAST, RegisterWindowMessage('KAW WHITE GESTOR'),0,0);
    MessageDlg('Sistema KAW WHITE GESTOR já está ativo!', mtInformation, [mbok],0);
    Halt(0);
  end
  else begin}
    Application.Initialize;
    Application.MainFormOnTaskbar := true;
    Application.Title := 'KAW WHITE GESTOR';
