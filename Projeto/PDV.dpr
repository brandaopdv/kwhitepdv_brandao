...
uses
  ...
  autoUpdate in '..\View\autoUpdate.pas',
  uUpdate in '..\View\uUpdate.pas' {FrmUpdate},
  uConsVendas in '..\View\uConsVendas.pas' {frmConsVendas},
  uSocketPing in '..\View\uSocketPing.pas',
  UConfigDatabase in '..\Model\UConfigDatabase.pas' {FrmConfigDatabase};

{$R *.res}

begin
  CreateMutex(nil,False,'KAW WHITE PDV');
  If GetLastError = ERROR_ALREADY_EXISTS Then
  Begin
    SendMessage(HWND_BROADCAST,
    RegisterWindowMessage('KAW WHITE PDV'),0,0);
    MessageDlg('Sistema KAW WHITE PDV já está ativo!', mtInformation, [mbok],0);
    Halt(0);
  End;
  Application.Initialize;
  Application.MainFormOnTaskbar := True;
  Application.Title := 'KAW WHITE PDV';
